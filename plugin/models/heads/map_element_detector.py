...
class MapElementDetector(DETRBboxHead):
    ...
    def forward(self, batch, context, img_metas=None, multi_scale=False):
        ...
        print("After _prepare_context:", sequential_context_embeddings.shape)
        ...
        print("After query_embedding:", query_embedding.shape)
        print("After bbox_embedding + view:", query_embedding.shape)

        img_masks = x.new_zeros((B, H, W))
        pos_embed = self.positional_encoding(img_masks)
        print("After positional_encoding:", pos_embed.shape)
        ...
        print("After transformer:", outs_dec.shape, init_reference.shape, inter_references.shape)
